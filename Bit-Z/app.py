...
def compress():
    ...
    if request.method == "GET":
        ...
        if len(up_file.filename) > 0:
            ...
            os.system('huffcompress.exe uploads\{}'.format(filename))
...
def decompress():
    ...
    if request.method == "GET":
        ...
        if len(up_file.filename) > 0:
            ...
            print(up_file.filename)
            up_file.save(os.path.join(app.config["FILE_UPLOADS"], filename))
            os.system('huffdecompress.exe uploads\{}'.format(filename))
            ...
            while True:
                if 'uploads\{}{}'.format(filename, ftype) in glob.glob('uploads/*-decompressed.*'):
                    os.system('move \"uploads\{}{}\" \"downloads\\ \"'.format(filename, ftype))
                    break
